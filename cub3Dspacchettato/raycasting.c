...
void	set_textured_floorceiling(win_data *win)
{
...
void	floor_ceiling(win_data *win)
{
...
void	ray_calculations(win_data *win)
{
...
void	set_directions(win_data *win)
{
...
void	set_text_variables(win_data *win)
{
	if (win->rayc.side == 0)
...
void	pre_draw_calculations(win_data *win)
{
	if (win->rayc.side == 0)
...
void	wall_casting(win_data *win, double *zbuffer)
{
	win->rayc.x = 0;
	while (win->rayc.x < win->resx)
	{
...
void	sprite_position(win_data *win)
{
	win->rayc.spritex = win->spr[win->rayc.i].posx - win->rayc.posx;
	win->rayc.spritey = win->spr[win->rayc.i].posy - win->rayc.posy;
	win->rayc.invdet = 1.0 /
	(win->rayc.planex * win->rayc.diry - win->rayc.dirx * win->rayc.planey);//required for correct matrix multiplication
	...
	win->rayc.planex * win->rayc.spritey);//this is actually the depth inside the screen, that what Z is in 3D
	win->rayc.sprscrx = (int)((win->resx / 2) *
	(1 + win->rayc.transformx / win->rayc.transformy));
}

void	sprite_calculations(win_data *win)
{
	sprite_position(win);
...
void	draw_sprites(win_data *win, double *zbuffer)
{
	win->rayc.i = 0;
	while (win->rayc.i < win->countsprite)
	{
		...
		{
			win->rayc.texx = (int)(256 * (win->rayc.stripe - (-win->rayc.sprw / 2 + win->rayc.sprscrx)) * win->text[4].w / win->rayc.sprw) / 256;
			if (win->rayc.transformy > 0 && win->rayc.stripe > 0 && win->rayc.stripe < win->resx && win->rayc.transformy < zbuffer[win->rayc.stripe])
			{
				win->rayc.y = win->rayc.drawstarty;
				while (win->rayc.y < win->rayc.drawendy)
				{
					win->rayc.d = (win->rayc.y) * 256 - win->resy * 128 + win->rayc.sprh * 128;
					win->rayc.texy = ((win->rayc.d * win->text[4].h) / win->rayc.sprh) / 256;
					win->rayc.color = (unsigned int)win->spr[win->rayc.i].addr[win->text[4].w * win->rayc.texy + win->rayc.texx];
					if ((win->rayc.color & 0x00FFFFFF) != 0) 
						draw_dot(win,win->rayc.stripe,win->rayc.y,win->rayc.color);
					win->rayc.y++;
				}
			}
			win->rayc.stripe++;
